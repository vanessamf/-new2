~function($){
	// 加载页面：jQuery $.ajax
	function ajaxLoad($ele,local){
		var html_obj = $.ajax({
			url:local,
			async:false
		});
		$ele.html( html_obj.responseText);

		// $ele.load(local,function(){})
	}
	$(document).ready(function(){
		...
		sub_nav.on('click','li > a',function(e){
			...
			/* // 对应的tab内容选项
			 if(flag_already){
			 $tab_content.find('.vbox').each(function(vin,vele){
			 if( tab_id === $(vele).data('itemid')){
			 $(vele).addClass('cur-show');return false;
			 }
			 })
			 }*/
			...
			if(!flag_already){
				...
				ajaxLoad( $vbox,target_url); //引入相应页面
