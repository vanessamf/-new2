...
(function($) {
	...
	$('.category-box').on('click', '#cre_menu_category', function(e) {
		var c_ul = $(this).parent().find('.category-op-list'),
			...
			'<span class="category-name">请命名<input class="cg-ipt input-sm" name="category-name" value="" readonly/></span>' +
	...
	category_op.on('click.category-op-item', '.category-op-item', function(e) {
		var cur_li = $(this)
	...
	//        针对 .menu-mg-tb表格 某一行菜品的操作
	var menu_mg_tb = $('.menu-mg-tb'),
		wrapper = $('.menu-main-wrapper .scrollable')
	menu_mg_tb.on('click', '.btn-op', function(e) {
		e.stopPropagation();
		e.preventDefault();
		...
		if($this.hasClass('op-up')) {
			...
			//复制菜品
			var first_td = menu_mg_tb.find('tbody tr:eq(0)'),
				clone_td = $tr.clone()

			wrapper.animate({
				scrollTop: 0
			}, 500);
			clone_td.find('td').eq(3).text('停用')
			clone_td.insertBefore(first_td).fadeOut().fadeIn();

		} else if($this.hasClass('op-delete')) {
			//删除当前单元格
			$tr.remove();
		} else if($this.hasClass('op-top')) {
			//置顶
			wrapper.animate({
				scrollTop: 0
			}, 500);
			$tr.fadeOut().fadeIn();
			menu_mg_tb.prepend($tr);
		} else if($this.hasClass('op-stick')) {
			//置底
			wrapper.animate({
				scrollTop: wrapper.height()
			}, 500)
			$tr.fadeOut().fadeIn();
			menu_mg_tb.append($tr);
		}
	});
	...
	// 添加配料组别
	$set_ingredients.on('click', '.add-menu-ingredients', function(e) {
		var $this = $(this),
			$dl = $this.parent().prev(),
			tpl_attr_ul = '<dd><ul class="menu-attr-list">' +
			'<li class="attr-name"><label class="wd-md control-label">配料组别：</label>' +
			' <div class="pull-left wd-sm">' +
			'<div class="btn-group sel-add-group m-r select-base">' +
			'<button data-toggle="dropdown" class="select-btn  dropdown-toggle">' +
			'<span class="dropdown-label">请选择</span>' +
			...
			'<li><a href="#"><input type="radio" name="d-s-r">辣度</a></li></ul></div></div></li>' +
			' <li class="attr-subdivision">' +
			'<label class="control-label">配料<span class="cell-tip">（至少填写两个）</span>：</label>' +
			' <input type="text" name="sub-attr" class="sub-a-cell input-sm" /><input type="text" name="sub-attr" class="sub-a-cell input-sm" /><input type="text" name="sub-attr" class="sub-a-cell input-sm" /> <input type="text" name="sub-attr" class="sub-a-cell input-sm" /><input type="text" name="sub-attr" class="sub-a-cell input-sm" /></li>' +
			'<li class="attr-item-ingredient"><i class="ingredient-ico-add"></i></li> <li class="attr-item-del"><i class="menu-ico-del"></i></li></ul></dd>';

		$dl.append(tpl_attr_ul)
	})

	//添加细分 配料
	$set_ingredients.on('click', '.attr-item-ingredient', function(e) {
		var _this = $(this),
			subdivision_list = _this.prev(),
			subdivision_item = '<input type="text" name="sub-attr" class="sub-a-cell input-sm" />'

		subdivision_list.append(subdivision_item)
	})
	// 删除配料组别
	$set_ingredients.on('click', '.attr-item-del', function(e) {
		var $this = $(this),
			$parent_dd = $this.closest('dd'),
			$parent_dl = $parent_dd.closest('.menu-attr-group'),
			dl_len = $parent_dl.children().length
